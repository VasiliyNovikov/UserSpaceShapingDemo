...
public sealed class ForwardingChannel : IDisposable
{
    ...
    public XdpForwarderMode Mode { get; }
    ...
    [SkipLocalsInit]
    public ForwardingChannel(string eth1, string eth2, XdpForwarderMode mode = XdpForwarderMode.Generic)
    {
        Mode = mode;
        SocketMode = mode is XdpForwarderMode.Generic ? XdpSocketMode.Default : XdpSocketMode.Driver;
        BindMode = mode is XdpForwarderMode.DriverZeroCopy ? XdpSocketBindMode.ZeroCopy : XdpSocketBindMode.Copy;
        Memory = new UMemory();
